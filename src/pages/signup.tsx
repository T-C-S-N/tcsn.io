...
export default function SignUpPage() {
  ...
  const handleSubmit = (e: any) => {
    ...
    axios.post('/api/auth/signup', formData)
      .then(res => router.push('/email-confirmation'))
      .catch(err => {
        setIsLoading(false)
        ...
        {!isLoading && (
          <form className='container width-90 sm-width-50 lg-width-30'>
            ...
            <div className='flex-column margin-vertical-10 width-100'>
              <a className='btn dark width-100' onClick={handleSubmit}>Sign Up</a>
            </div>
            <div className='flex-column margin-vertical-10 width-100'>
              <Link href='/signin' className='btn link width-100'>Already registered ?</Link>
            </div>
          </form>
        )}
      </main>
